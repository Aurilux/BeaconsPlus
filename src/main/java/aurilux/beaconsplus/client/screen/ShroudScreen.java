    ...
    public void tick() {
        ...
        if (this.buttonsNotDrawn && levels >= 0) {
            ...
            for (int powerLevel = 0; powerLevel < 3; powerLevel++) {
                int numEffects = ShroudTile.EFFECTS_LIST[powerLevel].length;
                int xOffset = numEffects * 22 + (numEffects - 1) * 2;

                for (int effectIndex = 0; effectIndex < numEffects; effectIndex++) {
                    Effect effect = ShroudTile.EFFECTS_LIST[powerLevel][effectIndex];
            ...
            int numEffects = ShroudTile.EFFECTS_LIST[3].length;
            int xOffset = (numEffects + 1) * 22 + numEffects * 2;

            for (int effectIndex = 0; effectIndex < numEffects; effectIndex++) {
                Effect effect = ShroudTile.EFFECTS_LIST[3][effectIndex];
