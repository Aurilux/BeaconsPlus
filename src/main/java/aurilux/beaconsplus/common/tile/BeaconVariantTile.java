package aurilux.beaconsplus.common.tile;
...
public abstract class BeaconVariantTile  extends TileEntity implements INamedContainerProvider, ITickableTileEntity {
    ...
    private List<BeaconVariantTile.BeamSegment> beamSegments = Lists.newArrayList();
    private List<BeaconVariantTile.BeamSegment> segmentSize = Lists.newArrayList();
    ...
    private final IIntArray intArray = new IIntArray() {
        public int get(int index) {
            switch(index) {
                case 0:
                    return BeaconVariantTile.this.levels;
                case 1:
                    return Effect.getId(BeaconVariantTile.this.primaryEffect);
                case 2:
                    return Effect.getId(BeaconVariantTile.this.secondaryEffect);
                default:
                    return 0;
            }
        }
        ...
        public void set(int index, int value) {
            switch(index) {
                case 0:
                    BeaconVariantTile.this.levels = value;
                    break;
                case 1:
                    if (!(BeaconVariantTile.this.world != null && BeaconVariantTile.this.world.isRemote) && !BeaconVariantTile.this.beamSegments.isEmpty()) {
                        BeaconVariantTile.this.playSound(SoundEvents.BLOCK_BEACON_POWER_SELECT);
                    }
                    BeaconVariantTile.this.primaryEffect = BeaconVariantTile.isBeaconEffect(value);
                    break;
                case 2:
                    BeaconVariantTile.this.secondaryEffect = BeaconVariantTile.isBeaconEffect(value);
            }
    ...
    public BeaconVariantTile(TileEntityType<?> tileEntityTypeIn) {
    ...
    public void tick() {
        ...
        BeaconVariantTile.BeamSegment beaconBeamSegment = this.segmentSize.isEmpty() ? null : this.segmentSize.get(this.segmentSize.size() - 1);
        ...
        for(int i = 0; i < 10 && blockpos.getY() <= height; i++) {
            ...
            if (colors != null) {
                if (this.segmentSize.size() <= 1) {
                    beaconBeamSegment = new LushTile.BeamSegment(colors);
                    ...
                    if (Arrays.equals(colors, beaconBeamSegment.colors)) {
                        ...
                        beaconBeamSegment = new LushTile.BeamSegment(new float[]{(beaconBeamSegment.colors[0] + colors[0]) / 2.0F, (beaconBeamSegment.colors[1] + colors[1]) / 2.0F, (beaconBeamSegment.colors[2] + colors[2]) / 2.0F});
                        this.segmentSize.add(beaconBeamSegment);
                    }
                }
            }
    ...
    @OnlyIn(Dist.CLIENT)
    public List<LushTile.BeamSegment> getBeamSegments() {
    ...
    public Container createMenu(int id, @Nonnull PlayerInventory playerInv, @Nonnull PlayerEntity player) {
        return LockableTileEntity.canUnlock(player, this.lock, this.getDisplayName()) ? new LushContainer(id, playerInv, this.intArray, IWorldPosCallable.of(Objects.requireNonNull(this.world), this.getPos())) : null;
    }

    @Nonnull
    @Override
    public ITextComponent getDisplayName() {
        return this.customName != null ? this.customName : new TranslationTextComponent("block.beaconsplus.lush");
    }
